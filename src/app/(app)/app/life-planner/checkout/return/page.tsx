...
function toCheckoutError(code: string): never {
  redirect(`/app/life-planner/checkout?error=${code}`);
}

/**
 * PayPal's own redirect target after a customer approves (or is asked to
 * approve) payment for Life Planner - the fallback path for a payment
 ...
 * navigates away from the checkout page. Delegates the actual "verify with
 * PayPal and finalize" work to `verifyAndFinalizeOrder`, the same shared,
 * idempotent logic `/api/webhooks/paypal` and every other checkout return
 * flow in this app use - same shape as `travel-planner/checkout/return/page.tsx`.
 *
...
export default async function CheckoutReturnPage({ searchParams }: CheckoutReturnPageProps) {
  ...
  switch (result.status) {
    case "paid":
    case "already-paid":
      redirect("/app/life-planner/checkout/success");
      break;
    case "not-eligible":
      toCheckoutError("order_not_found");
      break;
