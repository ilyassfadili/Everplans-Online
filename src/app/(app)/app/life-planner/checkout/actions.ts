...
/**
 * Life Planner's own in-page checkout Server Actions (Prompt 6 Phase 1) -
 * the same shape `travel-planner/checkout/actions.ts` already establishes,
 * reusing every underlying commerce primitive (`createPendingOrder`,
 * `createPayPalOrder`, `verifyAndFinalizeOrder`,
 ...
 * PayPal API calls, or payment verification, only Life Planner's own
 * product identity (`LIFE_PLANNER_PRODUCT`) and redirect paths.
 */
...
export async function createLifePlannerPayPalOrder(): Promise<CreateCheckoutOrderResult> {
  ...
  try {
    const order = await createPendingOrder({
      userId: user.id,
      plannerId: LIFE_PLANNER_PRODUCT.plannerId,
      productSlug: LIFE_PLANNER_PRODUCT.slug,
      productName: LIFE_PLANNER_PRODUCT.name,
      unitAmountCents: LIFE_PLANNER_PRODUCT.priceCents,
    ...
    const paypalOrder = await createPayPalOrder({
      ...
      returnUrl: `${origin}/app/life-planner/checkout/return`,
      cancelUrl: `${origin}/app/life-planner/checkout/cancel`,
    });

    await attachPayPalOrder(order.id, paypalOrder.id);
    return { status: "success", paypalOrderId: paypalOrder.id };
  } catch (error) {
    console.error("createLifePlannerPayPalOrder: failed to start checkout", error);
 ...
 * handler, and every other checkout in this app uses. The client never
 * learns anything more specific than "paid" or "error, try again" - it
 * has no say in whether the order actually becomes paid.
 */
export async function captureLifePlannerOrder(paypalOrderId: string): Promise<CaptureCheckoutOrderResult> {
  const user = await requireUser();
