...
/**
 * PayPal's own "customer cancelled" redirect target for Travel Planner's
 * checkout - marks the corresponding order `cancelled` on a best-effort
 * basis. A customer landing here without ever completing PayPal's
 * checkout never had a payment to lose, so there's nothing to verify,
 * only local bookkeeping to tidy up. No entitlement is ever granted from
 * this path. Same shape as `budget-planner/checkout/cancel/page.tsx`.
 */
  ...
  return (
    <Container size="narrow" className="flex flex-1 flex-col justify-center py-10 md:py-14">
      <EmptyState
        ...
        action={<Button href="/app/travel-planner/checkout">Try checkout again</Button>}
      />
    </Container>
  );
}
