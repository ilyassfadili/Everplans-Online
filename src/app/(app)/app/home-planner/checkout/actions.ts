...
/**
 * Home Planner's own in-page checkout Server Actions (Prompt 6 Phase 1) -
 * the same shape `travel-planner/checkout/actions.ts`/`life-planner/checkout/actions.ts`
 * already establish, reusing every underlying commerce primitive
 ...
 * PayPal API calls, or payment verification, only Home Planner's own
 * product identity (`HOME_PLANNER_PRODUCT`) and redirect paths.
 */
...
export async function createHomePlannerPayPalOrder(): Promise<CreateCheckoutOrderResult> {
  ...
  try {
    const order = await createPendingOrder({
      userId: user.id,
      plannerId: HOME_PLANNER_PRODUCT.plannerId,
      productSlug: HOME_PLANNER_PRODUCT.slug,
      productName: HOME_PLANNER_PRODUCT.name,
      unitAmountCents: HOME_PLANNER_PRODUCT.priceCents,
    ...
    const paypalOrder = await createPayPalOrder({
      ...
      returnUrl: `${origin}/app/home-planner/checkout/return`,
      cancelUrl: `${origin}/app/home-planner/checkout/cancel`,
    });

    await attachPayPalOrder(order.id, paypalOrder.id);
    return { status: "success", paypalOrderId: paypalOrder.id };
  } catch (error) {
    console.error("createHomePlannerPayPalOrder: failed to start checkout", error);
 ...
 * handler, and every other checkout in this app uses.
 */
export async function captureHomePlannerOrder(paypalOrderId: string): Promise<CaptureCheckoutOrderResult> {
  const user = await requireUser();
