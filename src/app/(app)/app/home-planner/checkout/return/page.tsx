...
function toCheckoutError(code: string): never {
  redirect(`/app/home-planner/checkout?error=${code}`);
}

/**
 * PayPal's own redirect target after a customer approves (or is asked to
 * approve) payment for Home Planner - the fallback path for a payment
 * method that needs a real redirect, even though the primary Card Fields/
 * Buttons flow never navigates away from the checkout page. Delegates the
 * actual "verify with PayPal and finalize" work to `verifyAndFinalizeOrder`,
 * the same shared, idempotent logic every other checkout in this app uses -
 * same shape as `travel-planner/checkout/return/page.tsx`.
 *
...
export default async function CheckoutReturnPage({ searchParams }: CheckoutReturnPageProps) {
  ...
  switch (result.status) {
    case "paid":
    case "already-paid":
      redirect("/app/home-planner/checkout/success");
      break;
    case "not-eligible":
      toCheckoutError("order_not_found");
      break;
